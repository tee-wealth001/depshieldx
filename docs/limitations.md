...
- deep mode depends on Docker being available (for npm, a small local `node:20` +
  ...
  built on first use) &mdash; see the [ecosystems overview](ecosystems/index.md)
...
- `Cargo.lock` parsing keeps only the newest resolved version when the same crate
  ...
  dropped &mdash; `packages.lock.json` parsing follows the same "keep the newest"
...
- some packages publish no PyPI or npm attestations at all; that is usually
  informational, not a red flag &mdash; Cargo/crates.io and Go modules have no
...
- npm's own "publish" attestation (signed with npm registry's own key, not a
  ...
  &mdash; only npm's SLSA provenance attestation is, since that's the one signed
  via GitHub Actions OIDC the same way PyPI's Trusted Publishing attestations are
- Cargo has no cryptographic provenance verification at all &mdash; crates.io
...
- Go modules have no separate per-package cryptographic provenance step either
  &mdash; checksum verification against the real checksum-transparency log
...
- Maven's Sigstore verification only recognizes one confirmed-real OIDC issuer
  (GitHub Actions) for now &mdash; a real signature from a different, equally
...
- a BOM import's version that's a `${property}` placeholder is resolved for Maven
  ...
  fuller cross-file property inheritance &mdash; an unresolvable entry is skipped
  rather than guessed at, and surfaces as a clear Maven error from the sandboxed
  build if it turns out to matter
- there is no Maven routing shim &mdash; see [Routing](cli/routing.md)
- NuGet's repository-signature check is presence-only, not a real
  certificate-chain validation &mdash; `depshieldx` has no trust-root story for
...
- both `dotnet add package` and `dotnet remove package` (and so `depshieldx`'s
  ...
  invocation &mdash; the real `dotnet` CLI itself doesn't accept more than one
  package name at a time
- Pub has no cryptographic signature verification at all &mdash; pub.dev has no
...
- deps.dev does not support Pub as an ecosystem at all &mdash; `depshieldx` skips
...
- Pub's behavioral tracing only covers Dart's Native Assets "hooks" mechanism
  (`hook/build.dart`) &mdash; Flutter-specific native platform plugin code
...
- RubyGems has no cryptographic signature verification at all &mdash; Sigstore
...
- a RubyGems version yanked long enough ago is removed outright from the
  ...
  against the real 2019 `rest-client` hijack incident) &mdash; since a 404 there
...
- `depshieldx` never runs `bundle install`/`bundle lock --local` on the host for
  RubyGems deep mode &mdash; `bundle install` triggers real native-extension
...
- platform-specific RubyGems variants (a gem publishing separate prebuilt
  binaries per OS/CPU) are out of scope &mdash; `depshieldx` always resolves and
...
- Composer has no cryptographic signature verification of any kind &mdash;
...
- deps.dev does not support Composer/PHP as an ecosystem at all, the same gap it
  has for Pub &mdash; `depshieldx` skips it explicitly rather than silently
...
- Composer's behavioral tracing only covers packages using the "files" autoload
  mechanism &mdash; Composer plugins (blocked by default as of 2.2+ unless the
...
- `depshieldx` never runs `composer install`/`composer update` on the host to
  build the Trivy-facing `composer.lock` for Composer deep mode &mdash; a
