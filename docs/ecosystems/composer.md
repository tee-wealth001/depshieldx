...
**A `composer.lock` file in the current directory** &mdash; auto-detected by
...
**One or more bare package names** &mdash; pass `--ecosystem composer` so
...
registry lookup &mdash; `composer require vendor/package` (with no version at all)
...
scratch-project resolve &mdash; `--no-install` skips straight past Composer's own
...
through `depshieldx install <package...> --ecosystem composer` &mdash; you don't
need to change your muscle memory.

"Install" here means `composer require` &mdash; adding the package(s) to your
...
packages in one call &mdash; so `depshieldx` pins every resolved package,
...
never produced by running `composer` on the host at all &mdash; a minimal,
...
reason" choice RubyGems makes (for an entirely different underlying reason &mdash;
...
says so plainly ("no checksum published &mdash; pinned by git reference only")
...
- `composer.json`-as-input &mdash; only `composer.lock` or bare package names via
  `--ecosystem composer` are accepted
- cryptographic signature verification of any kind &mdash; Packagist has no
...
- Composer plugins &mdash; as of Composer 2.2+, installing any package containing
