...
"Install" here means fetching every resolved coordinate &mdash; transitive
dependencies included, not just the ones you named &mdash; into your local repository
...
reverse. There is also no [routing shim](../cli/routing.md) for Maven &mdash; unlike
...
scanned with Trivy &mdash; Trivy reads the scratch `pom.xml` natively. The sandboxed
`mvn compile` is traced with `strace` for filesystem, process, and network activity
&mdash; see [Modes](../concepts/modes.md) for details. Unlike Cargo's `build.rs` or
...
it's present for &mdash; ordinary libraries that register no processor correctly
trace zero build-time activity.

Provenance checks for Maven combine checksum verification (SHA-256 where published,
falling back to SHA-1 for older releases &mdash; MD5 is never trusted), structural
...
Publisher Portal since January 2025) &mdash; see
[Provenance & Attestations](../concepts/provenance.md).

## Not supported

- `depshieldx uninstall` &mdash; see above, there's no well-defined manifest edit to
  reverse
- the [routing shim](../cli/routing.md) &mdash; Maven has no native "add a dependency"
  CLI command to intercept
- `pom.xml`-as-input &mdash; only explicit `groupId:artifactId[:version]` coordinates
