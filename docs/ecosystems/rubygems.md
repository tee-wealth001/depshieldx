...
**A `Gemfile.lock` file in the current directory** &mdash; auto-detected by filename,
no flag needed:
...
**One or more bare gem names** &mdash; pass `--ecosystem rubygems` so `depshieldx`
...
(rubygems.org-hosted gems) is resolved against the registry &mdash; `GIT`/`PATH`
...
`depshieldx install <gem...> --ecosystem rubygems` &mdash; you don't need to change
your muscle memory.

"Install" here means `bundle add` &mdash; adding the gem(s) to your project's
...
every named gem in the call &mdash; there's no way to pin more than one
...
ending up with every resolved gem &mdash; transitive included &mdash; pinned as a
...
`Gemfile.lock` &mdash; Trivy's Bundler support needs a real lock file, the same
...
&mdash; see [Modes](../concepts/modes.md) for details. RubyGems' real
code-execution surface is a native-extension gem's own `extconf.rb`, run as an
unavoidable part of installing it &mdash; not a separate later step the way Pub's
...
release) with a structural yank signal &mdash; see
...
enough ago is removed outright &mdash; confirmed against the real 2019
...
Unlike Pub, deps.dev does support RubyGems as an ecosystem &mdash; no explicit skip
needed here.

## Not supported

- `.gemspec`-as-input &mdash; only `Gemfile.lock` or bare gem names via
  `--ecosystem rubygems` are accepted
- cryptographic signature verification of any kind &mdash; rubygems.org's Sigstore
...
- platform-specific gem variants (a gem publishing separate prebuilt binaries per
  OS/CPU) &mdash; `depshieldx` always resolves and verifies against the
