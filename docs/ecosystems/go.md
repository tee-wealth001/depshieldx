...
**A `go.sum` file in the current directory** -- auto-detected by filename, no
flag needed:
...
**One or more bare module paths** -- pass `--ecosystem go` so `depshieldx` knows
...
graph -- `go.sum` alone can't reconstruct it, since it's a checksum allowlist (it
...
`depshieldx install <module> --ecosystem go` -- you don't need to change your
muscle memory.

"Install" here means `go get` -- adding the module(s) to your project's
`go.mod`/`go.sum` -- not `go install` (installing a binary program; since Go
...
a local file-based Go module proxy built on the host, and scanned with Trivy --
...
filesystem, process, and network activity -- see [Modes](../concepts/modes.md)
...
transparently inside the `go` toolchain itself during resolution -- what
...
Cargo's yanked-release signal -- see
[Provenance & Attestations](../concepts/provenance.md).

## Not supported

- installing binary programs (`go install`) -- `depshieldx`'s Go support only
  covers dependency modules added via `go get`
- `go.mod`-as-input -- only `go.sum` or bare module paths via `--ecosystem go`
  are accepted
- cryptographic provenance verification of the kind PyPI/npm have (per-package
  Sigstore signing) -- Go's real checksum-transparency verification already
