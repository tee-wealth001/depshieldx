...
built with PyInstaller. Download it, put it on `PATH`, and run it directly &mdash; no
...
That said, `depshieldx` doesn't reimplement `pip` or `npm` &mdash; it wraps the real
...
- Using it against **npm/yarn/pnpm** packages only requires Node.js/`npm` on the host
  &mdash; no Python needed at all, in either distribution.
- Using it against **Cargo/crates.io** packages only requires a Rust toolchain
  (`cargo`) on the host &mdash; no Python needed at all, in either distribution.
...
- Using it against **Maven/Maven Central** packages only requires a Java + Maven
  toolchain (`mvn`) on the host &mdash; no Python needed at all, in either
  distribution.
- Using it against **NuGet/NuGet.org** packages only requires a .NET SDK (`dotnet`) on
  the host &mdash; no Python needed at all, in either distribution.
- Using it against **Pub/pub.dev** packages only requires a Dart SDK (`dart`) on the
  host &mdash; no Python needed at all, in either distribution. Only the standalone
...
- Using it against **RubyGems/rubygems.org** packages only requires Ruby with Bundler
  (`bundle`) on the host &mdash; no Python needed at all, in either distribution.
...
- Using it against **Composer/Packagist** packages only requires PHP with Composer
  (`composer`) on the host &mdash; no Python needed at all, in either distribution.
...
Run `depshieldx doctor` at any time to check every prerequisite &mdash; the Python/pip
version gate, Docker daemon availability, host Trivy availability, and each
ecosystem's own toolchain on `PATH` &mdash; in one pass, so a missing toolchain shows
