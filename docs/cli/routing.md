...
- `cargo add <crate...>` &mdash; one or more crate names with no other flags,
  routed through `depshieldx install <crate...> --ecosystem cargo`. `cargo install`
  (binary crates) is not intercepted &mdash; depshieldx's cargo support only covers
  `cargo add`
- `go get <module...>` &mdash; one or more module paths with no other flags,
  routed through `depshieldx install <module...> --ecosystem go`. `go install`
  (binary programs) is not intercepted &mdash; depshieldx's Go support only covers
  `go get`
- `dotnet add package <name>` / `dotnet add package <name> --version <version>`
  &mdash; exactly one package, no other flags, routed through
  ...
  intercepted &mdash; anything beyond this exact shape passes straight through to
  the real `dotnet`
- `dart pub add <package...>` &mdash; one or more package names with no other
  ...
  intercepted &mdash; depshieldx's Pub support only covers hosted (pub.dev)
  packages, and anything using that syntax passes straight through to the real
  `dart`
- `bundle add <gem...>` (no other flags) &mdash; one or more gem names, routed
  ...
  gems &mdash; confirmed this is real Bundler behavior) is **not** intercepted,
  since that shape doesn't map to a safe per-gem depshieldx target; it passes
  straight through to the real `bundle`
- `composer require <package...>` (no other flags) &mdash; one or more
  ...
  aliases, not just exact versions &mdash; a colon-target whose constraint isn't a
...
There is no Maven shim, and none is planned &mdash; unlike `pip install`/
