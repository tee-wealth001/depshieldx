...
A block only happens when verification was actually attempted and failed &mdash; not
...
=== "npm / yarn / pnpm"
    ...
    - whether the release has npm provenance attestations, and whether its SLSA
      ...
      Fulcio certificate) verifies successfully &mdash; npm's separate "publish"
...
=== "Cargo"
    ...
    There is no cryptographic attestation infrastructure to check against &mdash;
    ...
    - crates.io's self-reported Trusted Publishing metadata (provider, repository,
      run ID), where present &mdash; recorded for reference only, since crates.io
...
=== "Go"
    ...
    transparently inside the `go` toolchain itself during resolution &mdash; there's
    ...
    - whether the resolved version has been retracted (a module author publishing a
      later version that lists a prior one as retracted &mdash; Go's closest
...
=== "Maven"
    ...
    - checksum verification (SHA-256 where Central publishes one, falling back to
      SHA-1 for older releases &mdash; MD5 is never trusted)
    - whether the resolved artifact has a PGP signature (Central has required these
      since the 2010s) &mdash; presence is recorded structurally only, since there's
    ...
    - whether the resolved artifact has a Sigstore bundle and, if so, whether it
      verifies successfully (a real cryptographic check, not just presence) &mdash;
...
=== "NuGet"
    ...
    - whether the resolved package has a repository signature &mdash; NuGet.org
...
=== "Pub"
    ...
    &mdash; pub.dev has no signing scheme of its own at all:
...
=== "RubyGems"
    ...
    signal &mdash; rubygems.org has no default, always-present signing scheme either
    ...
    - whether the resolved version has been yanked, where the registry still reports
      ...
      instead of staying queryable with a `yanked: true` flag &mdash; confirmed
...
=== "Composer"
    ...
    Provenance is structural more often than cryptographic &mdash; Packagist has no
    ...
    - checksum verification when Packagist's `dist.shasum` is actually published for
      the resolved version (a real SHA-1 hash comparison) &mdash; this is empty for
    ...
    - the git commit `reference` Packagist records for the resolved dist archive
      &mdash; the one real, always-present content pin regardless of whether a
