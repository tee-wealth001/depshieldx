...
from depshieldx.cli import _is_plain_npm_family_install, cli
...
class IsPlainNpmFamilyInstallTests(unittest.TestCase):
    ...
    def test_install_with_named_package_passes_through(self):
        # Not yet supported -- NpmEcosystem only resolves from an existing
        # lockfile, not an ad-hoc new package.
...
class RouteNpmFamilyCommandTests(unittest.TestCase):
    ...
    def test_route_npm_passes_through_when_naming_a_package(self):
        with tempfile.TemporaryDirectory() as temp_dir:
        ...
        self.assertEqual(result.exit_code, 0)
        mock_run.assert_called_once_with(["/usr/bin/npm", "install", "left-pad"], check=False)
