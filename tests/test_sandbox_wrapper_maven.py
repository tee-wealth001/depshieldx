...
from depshieldx.security.sandbox.sandbox_wrapper_maven import (
    BUNDLE_MOUNT_PREFIX,
    WORK_DIR,
    _build_verdicts,
...
class ClassifyWritePathTests(unittest.TestCase):
    def test_bundle_directory_write_is_tamper_attempt(self):
        self.assertEqual(
            _classify_write_path(f"{BUNDLE_MOUNT_PREFIX}/pom.xml"),
            "bundle_source_tamper_attempt",
        )

    def test_m2_repo_write_is_build_output(self):
        self.assertEqual(
            _classify_write_path(f"{WORK_DIR}/m2-repo/com/google/code/gson/gson/2.11.0/_remote.repositories"),
            "build_output",
        )

    def test_work_dir_write_is_project_files(self):
        self.assertEqual(_classify_write_path(f"{WORK_DIR}/target/classes/DepshieldxProbe.class"), "project_files")
...
class ParseStraceLogTests(unittest.TestCase):
    ...
    def test_remote_connect_is_recorded_as_blocked_network_event(self):
        log_path = self._write_log(
            ...
            'sin_addr=inet_addr("151.101.1.132")}, 16) = -1 ENETUNREACH (Network is unreachable)\n'
        )
    ...
    def test_write_into_bundle_directory_is_recorded_as_tamper_attempt(self):
        log_path = self._write_log(
            f'14   openat(AT_FDCWD, "{BUNDLE_MOUNT_PREFIX}/pom.xml", '
    ...
    def test_normal_target_write_is_not_blocked(self):
        log_path = self._write_log(
            f'14   openat(AT_FDCWD, "{WORK_DIR}/target/classes/DepshieldxProbe.class", '
    ...
    def test_execve_calls_are_deduplicated_into_subprocesses(self):
        log_path = self._write_log(
            '15   execve("/opt/java/openjdk/bin/java", ["java", "-classpath", "plexus-classworlds.jar"], 0x0 /* 1 vars */) = 0\n'
            '17   execve("/opt/java/openjdk/bin/java", ["java", "-classpath", "plexus-classworlds.jar"], 0x0 /* 1 vars */) = 0\n'
            '19   execve("/usr/bin/uname", ["uname"], 0x0 /* 1 vars */) = 0\n'
        )
        evidence = _parse_strace_log(log_path)
        self.assertEqual(evidence["syscall_counts"]["process_exec"], 3)
        self.assertEqual(len(evidence["subprocesses"]), 2)
        self.assertIn(["java", "-classpath", "plexus-classworlds.jar"], evidence["subprocesses"])
        self.assertIn(["uname"], evidence["subprocesses"])

    def test_non_syscall_lines_are_ignored(self):
        log_path = self._write_log("[INFO] Scanning for projects...\nnot a syscall line at all\n")
        evidence = _parse_strace_log(log_path)
        self.assertEqual(evidence["write_count"], 0)
...
class BuildVerdictsTests(unittest.TestCase):
    ...
    def test_network_attempt_produces_high_severity_verdict(self):
        ...
        verdicts = _build_verdicts(evidence, compile_exit_code=0)
    ...
    def test_build_failure_without_block_is_medium(self):
        evidence = self._empty_evidence()
        verdicts = _build_verdicts(evidence, compile_exit_code=1)
        codes = {v["code"]: v["severity"] for v in verdicts}
